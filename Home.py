...
if __name__ == '__main__':
    ...
    with st.sidebar:
        ...
        if 'req_mod' not in st.session_state:
        ...
        if drug_name and st.session_state.load_data and api_key and req_mod:
            with st.spinner('Fetching data.....'):
                ...
                st.session_state.subparts = subparts
                # st.session_state.flag = True
    ...
    with textcontainer:
        if "subparts" in st.session_state:
            selected = pills("Select the subpart you want to be Generated", list(st.session_state.subparts.keys()), index=None, clearable=True)
            if selected:
    ...
    if st.session_state.flag and st.session_state.curr_query and ("drug_name" in st.session_state) and ("req_mod" in st.session_state):
        ...
        with st.spinner("Finding refined queries....."):
            ...
            es_queries_dict = get_refined_query_es(st.session_state.curr_query, list(st.session_state.subparts.keys()), req_mod)
    ...
    if "es_queries_dict" in st.session_state and 'subparts' in st.session_state and not st.session_state.flag:
        # st.session_state.flag = True
        if len(st.session_state.es_queries_dict)==0:
            ...
            if selected_query == 'Other':
                ...
                with st.spinner('Generating new refined queries'):
                    queries_dict = get_refined_query(st.session_state.curr_query, list(st.session_state.subparts.keys()))
    ...
    if "queries_dict" in st.session_state and 'subparts' in st.session_state and not st.session_state.flag:
        st.session_state.flag = True
